...
const blockTypes: PageBlockType[] = ["hero", "rich_text", "image", "cta", "spacer"];
...
function normalizeBlock(block: unknown, index: number): PageBlock {
  if (!block || typeof block !== "object") throw new DomainError(`Block ${index + 1} is invalid.`);
  const candidate = block as { id?: unknown; type?: unknown; data?: unknown };
  ...
  return { id: textValue(candidate.id, 80) || randomUUID(), type, data };
}
...
export function normalizePageBlocks(input: unknown): PageBlock[] {
  ...
  return input.map(normalizeBlock);
}
