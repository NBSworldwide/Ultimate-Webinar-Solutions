...
Seeded samples never create login accounts or default passwords. To bootstrap the first local or production Administrator, set `INITIAL_ADMIN_EMAIL` and a strong `INITIAL_ADMIN_PASSWORD` in `.env.local`, optionally set `INITIAL_ADMIN_USERNAME`, then run `pnpm db:create-admin`; the command refuses to overwrite an existing admin. If the account already exists as a Customer, set `INITIAL_ADMIN_EXISTING_ACCOUNT_ONLY=true`; the one-time command promotes the matching email account, or the matching email-and-username account when a username is supplied, and refuses to make a different account when the values do not match. A username is required only when the command must create a new account. Customers and staff can sign in with either their email address or unique username. For role testing, set unique `DEMO_ADMIN_PASSWORD`, `DEMO_MANAGER_PASSWORD`, and `DEMO_CUSTOMER_PASSWORD` values of at least 20 characters plus `DEMO_USER_SEED_CONFIRMATION=local-only-demo-users` in the local environment, then run `pnpm db:seed-users`. That command creates or resets only the three reserved `.test` identities, requires explicit non-production `DEMO_MODE=true`, verifies the database is marked as the standalone dataset, records synthetic audit events, and never prints or stores passwords in plaintext.
...
- Role-based session authentication with HTTP-only cookies and email-or-username sign-in: administrators have full control, managers operate content, commerce, automated email, and visual appearance, and customers stay on public-facing account flows.
- Team & Access management with Administrator, Manager, and Customer roles; administrators can create staff and make direct staff changes, while Customer-to-Manager changes are approval requests routed to the Administrator of Record and fully audited. Approval or denial queues notifications for the customer, requester, and reviewing administrator; the last administrator cannot be demoted.
