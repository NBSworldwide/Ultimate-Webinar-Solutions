...
export default async function TeamPage() {
  ...
  return <div className="content-width">
    ...
    <section className="panel"><div className="panel-header"><div><h2 className="panel-title">Accounts <span className="muted">({users.length})</span></h2><span className="row-meta">Role changes and approval decisions are audited</span></div><UsersRound size={17} color="#0f776e" /></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Account</th><th>Role</th><th>Created</th><th>Change access</th></tr></thead><tbody>{users.map((user) => <tr key={user.id}><td><strong>{user.name}</strong><span className="muted" style={{ display: "block" }}>@{user.username} · {user.email}</span></td><td><span className={`status-badge status-${user.role === "admin" ? "active" : user.role === "manager" ? "queued" : "free"}`}><span className="status-dot" />{roleLabel(user.role)}</span></td><td className="muted">{formatDateTime(user.createdAt)}</td><td><TeamRoleForm userId={user.id} role={user.role} disabled={user.id === currentUser.id} canManageRoles={canManage} canRequestPromotion={hasCapability(currentUser, "team.promote")} promotionPending={pendingTargetIds.has(user.id)} /></td></tr>)}</tbody></table></div></section>
  </div>;
}
