...
import { getServiceLocationById, updateServiceLocation, type ServiceLocationInput } from "@/lib/service-locations";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const current = await getServiceLocationById(id, { includeUnpublished: true });
    if (!current) return NextResponse.json({ error: "Service location not found." }, { status: 404 });
    const location = await updateServiceLocation(id, serviceLocationSchema.parse(await request.json()) as ServiceLocationInput, user.id);
    revalidateLocationPaths(current.slug);
    ...
    return NextResponse.json({ error: "The service location could not be updated." }, { status: 500 });
  }
}
