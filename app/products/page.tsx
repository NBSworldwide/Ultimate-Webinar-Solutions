...
export default async function ProductsPage() {
  ...
  return <div className="public-shell"><EntityGraph data={buildProductIndexGraph(products, settings)} /><PublicHeader /><main className="public-main" id="main-content"><section className="public-hero"><span className="eyebrow">Physical goods</span><h1>Products that extend the session.</h1><p>Browse standalone merchandise, production gear, and attendee kits. Each item can be purchased independently from a webinar registration.</p></section><div className="notice-banner"><ShieldCheck size={17} /><span><strong>Fresh-start catalog.</strong> These products are synthetic samples with server-authoritative inventory. No archived product records are connected.</span></div>{products.length > 0 ? <section className="product-grid" aria-label="Product catalog">{products.map((product) => <ProductCard key={product.id} product={product} />)}</section> : <div className="empty-state panel"><div className="empty-icon"><Package size={20} /></div><h3>No products are available yet</h3><p>Use the admin workspace to add the first product.</p></div>}</main><PublicFooter /></div>;
}
