...
export function WebinarForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/admin/webinars", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title, eyebrow, description, startsAt: parsedStart.toISOString(), timezone, durationMinutes: Number(durationMinutes), hostName, tierName, priceCents: Math.round(Number(price) * 100), capacity: Number(capacity), status }) });
