...
import { orderNavigationMenus, primaryNavigationSlug, type NavigationCandidate, type NavigationLocation, type NavigationMenuItemView, type NavigationMenuView } from "@/lib/navigation";
...
const defaultMenuSlugs = new Set([primaryNavigationSlug, "footer-navigation"]);
...
export function NavigationManager({ menus, candidates }: { menus: NavigationMenuView[]; candidates: NavigationCandidate[] }) {
  ...
  const orderedMenus = orderNavigationMenus(menus);
  const initialMenu = orderedMenus.find((menu) => menu.slug === primaryNavigationSlug) ?? orderedMenus[0];
  const [menuList, setMenuList] = useState(() => orderedMenus);
  const [draft, setDraft] = useState<NavigationMenuView | null>(() => initialMenu ? cloneMenu(initialMenu) : null);
  ...
  function dropItem(targetId: string, mode: "inside" | "before" = "inside") {
    if (!draggedItemId || draggedItemId === targetId) return;
    const current = draft;
    const moved = current?.items.find((item) => item.id === draggedItemId);
    const target = current?.items.find((item) => item.id === targetId);
    if (!current || !moved || !target) return;
    if (descendantIds(moved.id, current.items).has(target.id)) {
      setDraggedItemId(null);
      setDropTargetKey(null);
      setStatus({ kind: "error", text: "A link cannot be nested inside its own subnav." });
      return;
    }
    setDraft((latest) => {
      if (!latest) return latest;
      const items = latest.items.filter((item) => item.id !== moved.id);
      const targetIndex = items.findIndex((item) => item.id === targetId);
      if (targetIndex < 0) return latest;
      if (mode === "before") {
        items.splice(targetIndex, 0, { ...moved, parentId: target.parentId });
      } else {
        const targetDescendants = descendantIds(targetId, items);
        const lastDescendantIndex = items.reduce((lastIndex, item, index) => targetDescendants.has(item.id) ? Math.max(lastIndex, index) : lastIndex, targetIndex);
        items.splice(lastDescendantIndex + 1, 0, { ...moved, parentId: targetId });
      }
      return { ...latest, items: normalizeOrder(items) };
    });
    setDraggedItemId(null);
    setDropTargetKey(null);
    setDirty(true);
    setStatus({ kind: "success", text: mode === "inside" ? `${moved.label} is now a subnav item under ${target.label}. Save the menu to publish it.` : `${moved.label} was moved before ${target.label}. Save the menu to publish it.` });
  }

  function beginDrag(event: DragEvent<HTMLElement>, itemId: string) {
  ...
  function handleDropOver(event: DragEvent<HTMLElement>, targetId: string, mode: "inside" | "before") {
    event.preventDefault();
    if (!draft || !draggedItemId || draggedItemId === targetId || descendantIds(draggedItemId, draft.items).has(targetId)) return;
    event.dataTransfer.dropEffect = "move";
    setDropTargetKey(`${mode}:${targetId}`);
  }
  ...
  return <div className="navigation-manager">
    <section className="navigation-manager-layout">
      <aside className="panel navigation-menu-list">
        <div className="panel-header"><div><span className="eyebrow">Saved menus</span><h2 className="panel-title">Menus</h2></div><span className="row-meta">{menuList.length}</span></div>
        {menuList.map((menu) => <button type="button" key={menu.id} className={`navigation-menu-list-item ${draft?.id === menu.id ? "active" : ""}`} onClick={() => selectMenu(menu)}><span><strong>{menu.name}</strong><small>{menu.locations.join(" · ")} · {menu.items.length} item{menu.items.length === 1 ? "" : "s"}</small></span><span className="status-dot" /></button>)}
        <form className="navigation-create-form" onSubmit={createMenu}>
          <h3>Create a menu</h3>
          <div className="field"><label htmlFor="navigation-new-name">Menu name</label><input id="navigation-new-name" value={menuName} onChange={(event) => setMenuName(event.target.value)} placeholder="Campaign menu" required /></div>
          <fieldset className="navigation-location-grid"><legend>Theme locations</legend>{locationOptions.map((option) => <label className="check-field" key={option.value}><input type="checkbox" checked={menuLocations.includes(option.value)} onChange={(event) => setMenuLocations((current) => event.target.checked ? [...new Set([...current, option.value])] : current.filter((value) => value !== option.value))} /><span><strong>{option.label}</strong><small>{option.description}</small></span></label>)}</fieldset>
          <label className="check-field"><input type="checkbox" checked={menuAutoAdd} onChange={(event) => setMenuAutoAdd(event.target.checked)} /><span>Automatically add published pages</span></label>
          <button className="button button-small" type="submit" disabled={creating}><Plus size={14} />{creating ? "Creating…" : "Create menu"}</button>
        </form>
      </aside>

      {draft ? <form className="panel navigation-menu-editor" onSubmit={saveMenu}>
        <div className="panel-header navigation-editor-header"><div><span className="eyebrow">Menu editor</span><h2 className="panel-title">{draft.name}</h2><span className="row-meta">Drag a link onto another link to make it a subnav; use the arrows or drop-before zones to reorder.</span></div><div className="detail-actions"><button type="button" className="button button-secondary button-small" onClick={() => void deleteMenu()} disabled={deleting || defaultMenuSlugs.has(draft.slug)}><Trash2 size={14} />{deleting ? "Deleting…" : "Delete"}</button><button className="button button-small" type="submit" disabled={saving}><Save size={14} />{saving ? "Saving…" : "Save menu"}</button></div></div>
        <div className="navigation-editor-settings"><div className="form-row"><div className="field"><label htmlFor="navigation-menu-name">Menu name</label><input id="navigation-menu-name" value={draft.name} onChange={(event) => updateDraft({ name: event.target.value })} required /></div><div className="field"><label>Theme locations</label><div className="navigation-location-chips">{locationOptions.map((option) => <label className="check-field" key={option.value}><input type="checkbox" checked={draft.locations.includes(option.value)} onChange={(event) => updateDraft({ locations: event.target.checked ? [...new Set([...draft.locations, option.value])] : draft.locations.filter((value) => value !== option.value) })} /><span>{option.label}</span></label>)}</div></div></div><label className="check-field"><input type="checkbox" checked={draft.autoAddPublishedPages} onChange={(event) => updateDraft({ autoAddPublishedPages: event.target.checked })} /><span><strong>Automatically add published pages to this menu</strong><small>New public pages appear at the end. You can relabel, hide, nest, or reorder them afterward.</small></span></label></div>
        <div className="navigation-add-area"><div className="navigation-add-grid"><div className="field"><label htmlFor="navigation-candidate">Add a page, product, session, or system link</label><select id="navigation-candidate" value={candidateKey} onChange={(event) => { setCandidateKey(event.target.value); const candidate = candidates.find((item) => item.key === event.target.value); if (candidate) addCandidate(candidate); }}><option value="">Choose an item…</option>{[...candidateGroups.entries()].map(([group, groupCandidates]) => <optgroup key={group} label={group}>{groupCandidates.map((candidate) => <option key={candidate.key} value={candidate.key} disabled={selectedIdentities.has(candidateIdentity(candidate))}>{selectedIdentities.has(candidateIdentity(candidate)) ? `${candidate.label} (already added)` : candidate.label}</option>)}</optgroup>)}</select></div><div className="navigation-custom-form"><div className="field"><label htmlFor="navigation-custom-label">Custom link label</label><input id="navigation-custom-label" value={customLabel} onChange={(event) => setCustomLabel(event.target.value)} placeholder="Customer portal" /></div><div className="field"><label htmlFor="navigation-custom-href">Custom link URL</label><input id="navigation-custom-href" value={customHref} onChange={(event) => setCustomHref(event.target.value)} placeholder="/account or https://example.com" /></div><button className="button button-secondary button-small" type="button" onClick={addCustomItem}><Plus size={14} />Add custom link</button></div></div></div>
        <div className="navigation-items-heading"><div><span className="eyebrow">Sortable structure</span><h3>Menu items <span className="muted">({sortedItems.length})</span></h3></div><span className="row-meta">Drop on a row to nest underneath it. Drop on the dashed target above a row to move before it.</span></div>
        {sortedItems.length > 0 ? <div className="navigation-item-list">{sortedItems.map((item, index) => {
          const descendants = descendantIds(item.id, draft.items);
          const invalidDropTarget = !draggedItemId || draggedItemId === item.id || descendantIds(draggedItemId, draft.items).has(item.id);
          const canShowDropBefore = Boolean(draggedItemId && !invalidDropTarget);
          return <div className="navigation-dnd-slot" key={item.id}>
            {canShowDropBefore ? <div className={`navigation-drop-before ${dropTargetKey === `before:${item.id}` ? "active" : ""}`} onDragOver={(event) => handleDropOver(event, item.id, "before")} onDragLeave={() => setDropTargetKey((current) => current === `before:${item.id}` ? null : current)} onDrop={() => dropItem(item.id, "before")} aria-label={`Drop the dragged link before ${item.label}`}>Drop before {item.label}</div> : null}
            <article className={`navigation-item-row ${draggedItemId === item.id ? "dragging" : ""} ${dropTargetKey === `inside:${item.id}` && !invalidDropTarget ? "drop-target" : ""}`} draggable onDragStart={(event) => beginDrag(event, item.id)} onDragOver={(event) => handleDropOver(event, item.id, "inside")} onDragLeave={() => setDropTargetKey((current) => current === `inside:${item.id}` ? null : current)} onDrop={() => dropItem(item.id, "inside")} onDragEnd={clearDrag} aria-label={invalidDropTarget ? undefined : `Drop the dragged link under ${item.label} to make a subnav`} style={{ "--item-depth": itemDepth(item.id, draft.items) } as CSSProperties}>
              <div className="navigation-item-indent"><GripVertical size={16} aria-hidden="true" /><span className="navigation-item-number">{index + 1}</span></div>
              <div className="navigation-item-fields"><div className="form-row"><div className="field"><label htmlFor={`navigation-label-${item.id}`}>Label</label><input id={`navigation-label-${item.id}`} value={item.label} onChange={(event) => updateItem(item.id, { label: event.target.value })} /></div><div className="field"><label htmlFor={`navigation-href-${item.id}`}>Link URL</label><input id={`navigation-href-${item.id}`} value={item.href} onChange={(event) => updateItem(item.id, { href: event.target.value })} /></div></div><div className="form-row"><div className="field"><label htmlFor={`navigation-parent-${item.id}`}>Parent item</label><select id={`navigation-parent-${item.id}`} value={item.parentId ?? ""} onChange={(event) => updateItem(item.id, { parentId: event.target.value || null })}><option value="">Top level</option>{sortedItems.filter((candidate) => candidate.id !== item.id && !descendants.has(candidate.id)).map((candidate) => <option key={candidate.id} value={candidate.id}>{"— ".repeat(itemDepth(candidate.id, draft.items))}{candidate.label}</option>)}</select></div><div className="navigation-item-meta"><span className="status-badge status-draft"><span className="status-dot" />{itemTypeLabel(item.itemType)}</span>{item.autoAdded ? <span className="row-meta">Auto-added page</span> : null}</div></div><div className="navigation-item-toggles"><label className="check-field"><input type="checkbox" checked={item.isVisible} onChange={(event) => updateItem(item.id, { isVisible: event.target.checked })} /><span>Visible</span></label><label className="check-field"><input type="checkbox" checked={item.openInNewTab} onChange={(event) => updateItem(item.id, { openInNewTab: event.target.checked })} /><span>Open in new tab</span></label></div>{dropTargetKey === `inside:${item.id}` && !invalidDropTarget ? <span className="navigation-drop-hint">Release to make the dragged link a subnav item under this label.</span> : null}</div>
        ...
        })}</div> : <div className="empty-state navigation-empty"><Plus size={20} /><h3>This menu has no items yet</h3><p>Choose a saved page, product, session, or system link above, or add a custom URL.</p></div>}
        <div className="navigation-editor-footer"><span className="form-help">Private sessions and admin-only routes are never available in the public candidate list.</span><button className="button" type="submit" disabled={saving}><Save size={15} />{saving ? "Saving…" : "Save menu"}</button></div>
        {status ? <p className={status.kind === "success" ? "form-success" : "form-error"} role={status.kind === "success" ? "status" : "alert"}>{status.text}</p> : null}
      </form> : <section className="panel navigation-empty-editor"><ExternalLink size={22} /><h2>Create your first menu</h2><p>Use the form on the left to create a menu, then add saved pages, product collections, sessions, or custom links.</p></section>}
    </section>
  </div>;
}
